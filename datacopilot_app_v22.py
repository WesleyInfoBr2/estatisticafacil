...
# Pré-carregar recursos do NLTK para análise de sentimento
try:
...
# Também configurar um logger para a memória para exibição na interface
...
# Função para criar link de download para arquivo
def get_download_link(file_path, link_text):
    ...
    try:
        ...
        logging.error(f"Erro ao criar link de download: {str(e)}")
...
# Adicionar seção de logs na barra lateral
with st.sidebar:
    st.header("Logs e Diagnóstico")
    if st.button("Mostrar logs recentes"):
    ...
    # Verificar se há arquivos de log para download
    log_files = []
    if os.path.exists(log_dir):
        log_files = [f for f in os.listdir(log_dir) if f.endswith('.log')]
    
    if log_files:
        st.subheader("Download de Logs")
        for log_file_name in log_files:
            log_file_path = os.path.join(log_dir, log_file_name)
            st.markdown(get_download_link(log_file_path, f"Download {log_file_name}"), unsafe_allow_html=True)
                ...
                if run_code and code_editado.strip():
                    try:
                        ...
                        if isinstance(__builtins__, dict):
                            allowed_builtins = {k: v for k, v in __builtins__.items() if k in ["print", "len", "range", "list", "dict", "str", "int", "float", "bool", "True", "False", "None", "max", "min", "sum", "abs", "round", "all", "any", "isinstance", "getattr", "hasattr", "Exception", "ValueError", "TypeError", "KeyError", "IndexError", "sorted", "zip", "map", "filter", "enumerate", "reversed", "set", "tuple", "__import__"]}
                        else:
                            allowed_builtins = {k: getattr(__builtins__, k) for k in dir(__builtins__) if k in ["print", "len", "range", "list", "dict", "str", "int", "float", "bool", "True", "False", "None", "max", "min", "sum", "abs", "round", "all", "any", "isinstance", "getattr", "hasattr", "Exception", "ValueError", "TypeError", "KeyError", "IndexError", "sorted", "zip", "map", "filter", "enumerate", "reversed", "set", "tuple", "__import__"]}
